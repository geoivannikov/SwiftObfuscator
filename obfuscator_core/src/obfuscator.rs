...
/// Stores mapping from original class, struct, enum, protocol and typealias names to obfuscated names.
...
impl ObfuscationMap {
    ...
    pub fn from_contents(contents: &[String]) -> Result<Self, regex::Error> {
        ...
        // Finally collect all typealiases
        for content in contents {
            for caps in typealias_re.captures_iter(content) {
                let orig = &caps[1];
                mapping.entry(orig.to_string()).or_insert_with(|| {
                    let name = format!("ObfAlias{}", typealias_counter);
                    typealias_counter += 1;
                    name
                });
            }
    ...
    /// Obfuscates all class, struct, enum, protocol and typealias names in the given source string.
